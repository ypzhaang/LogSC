...
while t < pars.num_iters
    t=t+1;
        ...
        Ttt=distMat(S',S');
        [sameClass_Dis,diffClass_Dis]=computDis(10,50,Ttt);
        fprintf('sameClass_Dis/diffClass_Dis=%f\n',sameClass_Dis/diffClass_Dis);
       end
...
function [sameClass_Dis,diffClass_Dis]=computDis(nClass,nPerClass,X)
    sameClass_Dis=0;
    for ii=1:1:nClass
        Xi=X((ii-1)*nPerClass+1:ii*nPerClass,(ii-1)*nPerClass+1:ii*nPerClass);
        sameClass_Dis=sameClass_Dis+sum(Xi(:));
    end
    diffClass_Dis=sum(X(:))-sameClass_Dis;